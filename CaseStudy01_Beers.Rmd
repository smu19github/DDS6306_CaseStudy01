...
```
```{r}
fullData <- rename(fullData, BreweryName = Name.x, BeerName = Name.y) %>% write_csv(fullData)
names(fullData)
```
```{r}
fullData %>%
...
```{r}
names(sDF)
qsDF<-sDF[sort(sDF$Brew_Count),]
tsDF<-qsDF %>% 
  tibble::as_tibble(qsDF)
tail(tsDF)
print(qsDF)
```
```{r}
p3 <- sDF %>%
  plot_ly(x = ~NoBreweries, y = ~State ) %>%
  add_bars(marker = list(color = 'rgba(58, 71, 80, 0.6)')) %>%
  layout(title = "Count of Brewereies")
